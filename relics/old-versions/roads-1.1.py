...
def floyed():
	...
	for i in range(ltot):
		for j in range(len(l_dis[i])):
			...
			maps[l_route[i][j]][l_route[i][j+1]]=l_dis[i][j]+5
			maps[l_route[i][j+1]][l_route[i][j]]=l_dis[i][j]+5
...
Label(root,textvariable=tk_money).grid(row=3,column=2)
...
message('Roads version 1.1 by Mirpri, 2024','special')

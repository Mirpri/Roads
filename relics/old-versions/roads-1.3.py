...
def trolley(line,dire,level,atk=-1,pas=[-1],pstp=0):
    def printtag():
        if len(pas)<5:
            ...
            M.itemconfig(tag,text=pas[:3]+['..('+str(len(pas))+')'])
    global trains
    if pas==[-1]:
        pas=[]
    trains+=1
...
def message(text,tpe='normal'):
    ...
    if tpe=='warning':
        ...
        messageboxx.config(foreground='brown')
    elif tpe=='normal':
...
def load(path):
    global run,timet,rtot,ltot,money,gem,delivered,newtrlev,ktot,k_lis,k_x,k_y,k_level,k_full,k_occ,l_route,l_pos,l_color,l_box,l_ele,l_trnum,l_sch,l_dis,l_lv,pool,k_ids,k_tagids,k_txtids,l_tag,cities
